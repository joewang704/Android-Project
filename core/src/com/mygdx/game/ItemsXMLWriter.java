...
public class ItemsXMLWriter {
    public static void createItems() {
        //xml file for all items
        FileHandle xmlItems = Gdx.files.local("Items.xml");
        FileHandle internalXmlItems = Gdx.files.internal("Items.xml");

        //xml file for items the user has acquired
        FileHandle acquiredXmlItems = Gdx.files.local("AcquiredItems.xml");
        FileHandle acquiredInternalXmlItems = Gdx.files.internal("Items.xml");

        if (!(acquiredXmlItems.exists() && acquiredInternalXmlItems.equals(internalXmlItems)) &&
                Gdx.app.getType() == Application.ApplicationType.Android) {
            //for android copy Items.xml from internal directory into local directory
            acquiredInternalXmlItems.copyTo(Gdx.files.local(
                    acquiredInternalXmlItems.file().getPath()));
        }
